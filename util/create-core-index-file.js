...
const corePackageName = "powerjinja-core";

if (!fs.existsSync(path.resolve('packages/' + corePackageName))) {
    throw Error(`No package with name "${corePackageName}" found!`);
}

const imports = [];
const definitions = [];

const packageName = generateNameVariantsFromKebabCase(corePackageName);
...
let indexFile = `import { createInstance } from "./create-instance";\n\n`;
indexFile += imports.join('\n');
indexFile += `\n\nexport const ${packageName.camelCase} = createInstance();\n\n`;
